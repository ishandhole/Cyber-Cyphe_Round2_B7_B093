...
export function learn(
  ...
  if (recentFails.length >= 3) {
    ...
    // Type assertion correctly applied here to satisfy TypeScript
    (Object.keys(updatedT) as ActionType[]).forEach(k => {
      const from = updatedT[k];
      if (from !== undefined) {
        updatedT[k] = Math.min(0.93, from + 0.012);
        thresholdChanges.push({ action: k, from, to: updatedT[k], reason: `Model drift: ${recentFails.length} recent failures` });
      }
    });
  ...
  return { updatedThresholds: updatedT as Record<ActionType, number>, newAntibodies, updatedAntibodies, newOutcomes, notes, divergenceDelta, thresholdChanges };
}
